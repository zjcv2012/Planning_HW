...
static void symbolic_planner(int* blocksV, int numofblocks, int* trianglesV, int numoftriangles, int TableIndex, 
            ...
            int moveActionIndex, int moveToTableActionIndex, int*** plan, int* planlength) 
{
    ...
    *planlength = 0;
    ...
    symbPlanner planner(blocksV, numofblocks, trianglesV, numoftriangles, TableIndex, 
    ...
    vector<Action> symbPlan = planner.Plan();

    if(symbPlan.size()==0) return;
    
    int numofsteps = symbPlan.size();
    *plan = (int**) malloc(numofsteps*sizeof(int*));
    ...
    for (i = 0; i < numofsteps; i++)
    {
        ...
        (*plan)[i][0] = symbPlan[i].action_index;
        (*plan)[i][1] = symbPlan[i].action_first;
        (*plan)[i][2] = symbPlan[i].action_second; 
        (*plan)[i][3] = symbPlan[i].action_third;
...
static void dumb_planner(int* blocksV, int numofblocks, int* trianglesV, int numoftriangles, int TableIndex, 
            int** onV_start, int onV_start_length, int* clearV_start, int numofclear_start, 
            int** onV_goal, int onV_goal_length, int* clearV_goal, int numofclear_goal, 
            int moveActionIndex, int moveToTableActionIndex, int*** plan, int* planlength) 
{
...
{ 
    ...
    symbolic_planner(blocksV, numofblocks, trianglesV, numoftriangles, TableIndex, 
